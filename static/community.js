(function () {
  ...
  function bindMessages(){root.querySelectorAll("[data-message]").forEach(function(b){b.onclick=function(){var u;try{u=requireUser();}catch(x){return;}var other=b.dataset.message;if(other===u.uid)return;var card=b.closest(".social-card"),old=card.querySelector(".message-form");if(old){old.remove();return;}var form=document.createElement("form");form.className="message-form";form.innerHTML='<label>'+esc(labels.text)+'<textarea required maxlength="2000"></textarea></label><button class="btn sm" type="submit">'+esc(labels.join)+'</button><span role="status"></span>';card.appendChild(form);form.querySelector("textarea").focus();form.onsubmit=function(e){e.preventDefault();var text=form.querySelector("textarea").value.trim(),status=form.querySelector("span");if(!text)return;status.textContent="…";api().then(function(a){var ids=[u.uid,other].sort(),id=ids.join("_");var ref=a.M.db.doc(a.db,"conversations",id);return a.M.db.setDoc(ref,{uid:u.uid,memberIds:ids,updated:a.M.db.serverTimestamp()},{merge:true}).then(function(){return a.M.db.addDoc(a.M.db.collection(a.db,"conversations",id,"messages"),{uid:u.uid,text:text,created:a.M.db.serverTimestamp()});});}).then(function(){status.textContent="✓";form.querySelector("textarea").value="";}).catch(function(){status.textContent="!";});};};});}
