...
| 2026-08-30 | H — Day-trip finder | no way to answer "I'm free today, where do I go?" — the planner assumed you already knew | `/day-trip/` × 6 languages: start point (or geolocation), group size, interests, hours available → the best destination plus what else fits, and the car it needs | the highest-intent question a visitor has, and the bridge from planning to renting | `render_day_trip()`, `static/daytrip.js`, road model reused from `road_model()` | simulated against real data across 8 scenarios | ✅ shipped |
| 2026-08-30 | H — Drive-time model | drive times existed only from Tbilisi | any origin, using each place's own measured winding factor (median 1.48) and road speed (paved 65, mostly-paved 60, gravel 49, 4×4 42 km/h) | measured from 244 places' real road distance and drive time; labelled as an estimate in the UI, never as routed directions | `leg()` in `daytrip.js` | 8-scenario simulation | ✅ honest |
| 2026-08-31 | I — WebPage node | 168 of 2 359 pages declared what the page *is*; the other 2 191 emitted a bare `Car` or `TouristAttraction` with nothing tying it to its page, breadcrumb or language | 2 310 | a crawler had no `mainEntity`, `breadcrumb`, `isPartOf` or `inLanguage` to work from | `_add_webpage_node()` in `head_html()`, skipped where a page already declares its own type | JSON-LD scan of `dist/` | ✅ 168 → 2 310 |
| 2026-08-31 | I — hreflang on noindex | 18 excluded pages advertised a full six-language cluster | 0 | a noindex page is not part of any language cluster; listing it invited six excluded URLs as alternates | `_noindex()` guard in `head_html()` | `grep 'rel="alternate" hreflang' dist` on noindex pages | ✅ 0 |
| 2026-08-31 | I — Alias language | the 12 `/planner/` and `/pricing/` redirect stubs shipped no `<html lang>` at all | every stub declares its language and direction | a page that redirects into a language should say which one | stub template | `<html lang>` scan | ✅ 12 → 0 |
| 2026-08-31 | I — CI guard | the quality gate ran content, tests, JS, build and links — but not the SEO audit | `scripts/seo_audit.py` is stage 8 of 8 | nothing stopped a future change from silently reintroducing what this programme fixed; the audit exits non-zero only on ERROR, so warnings stay informational | `scripts/run_quality_gate.py` | all 8 stages run clean locally | ✅ added |
| 2026-08-31 | J — Monthly rental | no page for long-term rental, though `price_30` exists for all 17 cars and is 24-25% below the daily rate | `/car-rental/monthly/` × 6 languages with a computed per-category table (30+ rate, thirty-day total, saving, deposit) and `AggregateOffer` | ranked #2 of the twenty highest-value pages to build in `KEYWORD_CLUSTERS.md` | `render_rental_duration()`, `durations:` in `seo_car_rental.yml` | every cell recomputed from `content/cars/*.yml` | ✅ +6 pages |
| 2026-08-31 | J — Card image weight | cards served the full 900-1100px original at ~400 CSS px — one region page loaded **3 907 KB** of card photos | **1 089 KB** (-72%); median image 76 KB → 31 KB | the single biggest remaining Core Web Vitals cost | 480px variants generated into `dist/` only, never committed; `card_img()` / `img_srcset()` and the generator read one shared set so a page can never name a variant the build did not make | byte count over `dist/`; 0 broken `-480` references | ✅ -72% |
| 2026-08-31 | J — Wrong intrinsic size | every photo declared `width=1100 height=688` regardless of its real size (median is 900px wide) | real per-image dimensions | the browser reserved the wrong box, so the layout shifted when the image arrived | `photo_dims()` | rendered `<img>` inspection | ✅ fixed |
| 2026-08-31 | J — Dead excess link | `terms.yml` sent readers to `/pricing/` for the excess table in all 6 languages, but `/pricing/` redirects to `/fleet/`, which publishes no excess | link repointed to `/car-rental/`, where the reconciled terms table now lives | a reader following the site's own instruction found nothing | `content/pages/terms.yml` | `grep /pricing/ content/pages/terms.yml` → 0 | ✅ fixed |
| 2026-09-01 | K — Nobody told the engines | no Google, Bing or Yandex ownership tag on any page, so there was no way to know whether a single page is indexed, what it appears for, or where it ranks | `verification:` in `site.yml` emits each tag, and only for a code that is really set | the whole programme was running blind | `verification_html()` | tag present on 2 346 pages with a code, on 0 without | ✅ ready for the codes |
| 2026-09-01 | K — Analytics never ran | the GA4 id was read only from `GA_MEASUREMENT_ID`, which CI does not set, so analytics was off on every deploy since it was added | read from `site.yml` first, environment second | not even visitor counts existed | `analytics_html()` | `measurementId` in the built page | ✅ ready for the id |
| 2026-09-01 | K — IndexNow | a new site waits weeks for a first crawl | key file published by the build + `scripts/indexnow_submit.py` sends the sitemap to Bing, Yandex, Seznam and Naver, which fetch within hours | Google does not participate, so this complements Search Console rather than replacing it; Yandex matters for the Russian-language share of Georgian rental demand | `site.yml → indexnow_key`, `scripts/indexnow_submit.py` | script refuses cleanly when the key file is not yet live | ✅ shipped |
